...
void main() {
    ...
    for (uint i = 0u; i < uSteps; i++) {
        ...
        vec4 volumeSample;
        if (photon.position.y > 0.1f) {
            vec3 hitN = -photon.direction;
            float k = fresnel(photon.direction, hitN, refractionIndex);

            vec3 refractPos = vec3(0.f);

            if (k < 1.f) {
                refractPos = photon.position + t * normalize(refract(photon.direction, hitN, refractionIndex));
            }

            vec3 reflectPos = photon.position + t * normalize(reflect(photon.direction, hitN));

            volumeSample =  sampleVolumeColor(reflectPos) * k + sampleVolumeColor(refractPos) * (1.f - k);
        } else {
            volumeSample = sampleVolumeColor(photon.position);
        }
